...
/**
 * Service implementation providing the reducers for rankings.
 */
public class RankingReducerFactoryServiceProvider implements ReducerFactoryService {

  @SuppressWarnings("unchecked")
  @Override
  public <T> Factory<T> getFactory(Class<T> structureType) {
    if (Ranking.class.equals(structureType)) {
      return (Factory<T>) new Reducers.Factory<Ranking>() {

        @Override
        public RoleReducer<Ranking> join() {
          return new RankingJoin();
        }

        @Override
        public RoleReducer<Ranking> meet() {
          return new RankingMeet();
        }
