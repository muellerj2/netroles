...
public class Equivalences {
  ...
  /**
   * Computes the maximum relative regular equivalence for a network with categorical data on links.
   ...
   *          equivalence[i] <= max_{1<=j<=i-1} p[j] + 1
   * @param edgeClasses
   *          defines what edge class each edge is in; relatively regular equivalences require that
   *          equivalent vertices are connected to equivalent neighbors via edges of the same class;
   *          the algorithm here assumes that neighborhoods are sorted increasingly by edge class
   * @return an array representing the maximum relative regular equivalence: vertices that have the
   *         same associated number in the array (based on their IDs) are structurally equivalent.
   *         The algorithm guarantees the following properties of the output array: p[0] = 0 and
   *         p[i] <= max_{1<=j<=i-1} p[j] + 1
  ...
  public static Mapping.OfInt relativeRegularEquivalence(int n,
    ...
    refiningRegularEquivalenceImpl(n, positionView, equivalenceRelativeTo, edgeClasses, state);
  ...
  /**
   * Computes the maximum relative equivalence refining another equivalence for a network with
   * categorical data on links. Space: O(n + m), Runtime: O(n + m log n) worst case.
   *
   * @param n
   *          Number of nodes
   * @param positionView
  ...
  public static Mapping.OfInt refiningRelativeRegularEquivalence(int n,
      NetworkView<?, ?> positionView, ConstMapping.OfInt equivalenceRelativeTo,
      ConstMapping.OfInt equivalenceToRefine, int[] edgeClasses) {
    checkEquivalenceFormat(equivalenceRelativeTo, EQUIVALENCE_RELATIVE_TO);
    checkEquivalenceFormat(equivalenceToRefine, EQUIVALENCE_TO_REFINE);
  ...
  private static <T> void refiningRegularEquivalenceImpl(final int n,
      NetworkView<?, T> positionView, ConstMapping.OfInt equivalenceRelativeTo,
      int[] edgeClasses, final EquivalenceAlgorithmState state) {
    ...
    final Iterator<? extends T>[] iterators = (Iterator<? extends T>[]) new Iterator[maxNumberVerticesForColor];
    @SuppressWarnings("unchecked")
    final T[] firstRelationships = (T[]) new Object[maxNumberVerticesForColor];
    ...
    while (begin < n) {
      ...
      for (int iv = end; iv < n; end = ++iv) {
        ...
        iterators[iv - begin] = positionView.inverseTies(v).iterator();
        if (iterators[iv - begin].hasNext()) {
          T r = firstRelationships[iv - begin] = iterators[iv - begin].next();
          heap.upsert(iv, edgeClasses[positionView.inverseTieIndex(v, r)]);
        }
      }
      int expectedclass = !heap.isEmpty()
          ? edgeClasses[positionView.inverseTieIndex(verticesOrderedByColor[heap.peek()],
              firstRelationships[heap.peek() - begin])]
          : 0;
      ...
      while (!heap.isEmpty()) {
        ...
        T r = firstRelationships[iv - begin];
        int v = verticesOrderedByColor[iv];
        while (r != null && edgeClasses[positionView.inverseTieIndex(v, r)] == expectedclass) {
          final int iw = positionView.inverseTieTarget(v, r);
          final int oldcolor = state.getColor(iw);
        ...
        // do clean-up on processed colors
        // and garbage-collect unused colors
        int nextclass = !heap.isEmpty()
            ? edgeClasses[positionView.inverseTieIndex(verticesOrderedByColor[heap.peek()],
                firstRelationships[heap.peek() - begin])]
            : 0;
        if (heap.isEmpty() || nextclass != expectedclass) {
          expectedclass = nextclass;
          for (int i = 0; i < state.getProcessedList().size(); ++i) {
            final int oldcolor = state.getProcessedList().getInt(i);
            // if color unused -> free it
            if (state.getCount(oldcolor) == 0) {
   ...
   *          defines what edge class each edge is in; relatively regular equivalences require that
   *          equivalent vertices are connected to equivalent neighbors via edges of the same class;
   *          the algorithm here assumes that neighborhoods are sorted increasingly according to the
   *          standard integer ordering by edge class (and not according to the ordering imposed by
   *          the comparator)
   * @param comparator
   *          Compares the edge classes. Result must be Integer.MIN_VALUE if incomparable, positive
   *          if left argument is greater and negative if it is less than the right argument. Note
   *          that edge class equivalence (result 0) is not allowed and not handled correctly by the
   *          algorithm.
   * @return an array representing the maximum relative regular equivalence: vertices that have the
   *         same associated number in the array (based on their IDs) are structurally equivalent.
   *         The algorithm guarantees the following properties of the output array: p[0] = 0 and
  ...
  public static Mapping.OfInt relativeRegularEquivalence(int n,
    ...
    refiningRegularEquivalenceImpl(n, positionView, equivalenceRelativeTo, edgeClasses, comparator,
        state);
  ...
  public static Mapping.OfInt refiningRelativeRegularEquivalence(int n,
      NetworkView<?, ?> positionView, ConstMapping.OfInt equivalenceRelativeTo,
      ConstMapping.OfInt equivalenceToRefine, int[] edgeClasses, IntBinaryOperator comparator) {
    checkEquivalenceFormat(equivalenceRelativeTo, EQUIVALENCE_RELATIVE_TO);
    checkEquivalenceFormat(equivalenceToRefine, EQUIVALENCE_TO_REFINE);
  ...
  private static <U, T> void refiningRegularEquivalenceImpl(final int n,
      final NetworkView<U, T> positionView, final ConstMapping.OfInt equivalenceRelativeTo,
      int[] edgeClasses, IntBinaryOperator comparator, final EquivalenceAlgorithmState state) {
    ...
    final boolean[] includedEdges = new boolean[edgeClasses.length * 2];
    boolean[] colorEncounteredBefore = new boolean[n];

    for (int i = 0; i < n; ++i) {

      for (U r : positionView.ties(i)) {
        int color = equivalenceRelativeTo.getInt(positionView.tieTarget(i, r));
        int edgeClass = edgeClasses[positionView.tieIndex(i, r)];
        if (!colorEncounteredBefore[color]) {
          state.getProcessedList().addInt(color);
          state.setColorMappedToValue(color, edgeClass);
          colorEncounteredBefore[color] = true;
        } else if (comparator.applyAsInt(edgeClass, state.getColorMappedToValue(color)) > 0) {
          state.setColorMappedToValue(color, edgeClass);
        }
      }
      ...
      for (U r : positionView.ties(i)) {
        ...
        if (edgeClass == state.getColorMappedToValue(color)) {
      ...
      for (int j = 0; j < state.getProcessedList().size(); ++j) {
        ...
        state.setColorMappedToValue(color, -1);
        colorEncounteredBefore[color] = false;
      }
      state.getProcessedList().clear();
    }

    refiningRegularEquivalenceImpl(n,
        // only provides implementations for the necessary methods
        new NetworkView<U, T>() {

          @Override
          public Iterable<? extends U> ties(int node) {
            throw new UnsupportedOperationException();
          }

          private <V> Iterable<V> fromIterator(Iterator<V> it) {
            return () -> it;
          }

          @Override
          public Iterable<? extends T> inverseTies(int node) {
            return fromIterator(
          ...
          @Override
          public int tieTarget(int node, U tie) {
            throw new UnsupportedOperationException();
          }

          @Override
          public int inverseTieTarget(int node, T inverseTie) {
            return positionView.inverseTieTarget(node, inverseTie);
          }

          @Override
          public int tieIndex(int node, U tie) {
            throw new UnsupportedOperationException();
          }

          @Override
          public int inverseTieIndex(int node, T inverseTie) {
          ...
          @Override
          public int uniqueTieIndex(int node, U tie) {
            return tieIndex(node, tie);
          }

          @Override
          public int uniqueInverseTieIndex(int node, T inverseTie) {
        ...
        }, equivalenceRelativeTo, edgeClasses, state);
  }
   ...
   * @param comparatorExtension
   * @return an array representing the maximum relative regular equivalence: vertices that have the
   *         same associated number in the array (based on their IDs) are structurally equivalent.
   *         The algorithm guarantees the following properties of the output array: p[0] = 0 and
   *         p[i] <= max_{1<=j<=i-1} p[j] + 1
   */
  public static int[] maximumRelativeRegularEquivalenceExtended(final Network g,
      final int[] equivalenceRelativeTo, Direction dir,
      Comparator<Relationship> comparatorExtension) {
    checkEquivalenceFormat(equivalenceRelativeTo, EQUIVALENCE_RELATIVE_TO);

    final int[] resEquivalence = new int[equivalenceRelativeTo.length];
    final int n = equivalenceRelativeTo.length;
    final int[] counts = new int[2 * n];
    counts[0] = n;
    ...
    coarsestRefiningEquivalenceRelativeToExtended(g, state, equivalenceRelativeTo, dir,
        comparatorExtension);

    normalizePartition(state.getEquivalence(), state.getColorsMap());

    return state.getEquivalence();
  }
   ...
   *         properties of the output array: p[0] = 0 and p[i] <= max_{1<=j<=i-1} p[j] + 1
   */
  public static int[] maximumRefiningRelativeEquivalenceExtended(final Network net,
      final int[] equivalenceRelativeTo, final int[] equivalenceToRefine, Direction dir,
      Comparator<Relationship> comparatorExtension) {
    checkEquivalenceFormat(equivalenceRelativeTo, EQUIVALENCE_RELATIVE_TO);
    checkEquivalenceFormat(equivalenceToRefine, EQUIVALENCE_TO_REFINE);

    final int[] resEquivalence = Arrays.copyOf(equivalenceToRefine, equivalenceToRefine.length);
    final int n = net.asRelation().countUnionDomain();
    final int[] counts = new int[2 * n];
    int maxColor = 0;
    for (final int c : equivalenceToRefine) {
      maxColor = Math.max(c, maxColor);
    ...
    coarsestRefiningEquivalenceRelativeToExtended(net, state, equivalenceRelativeTo, dir,
        comparatorExtension);
  ...
  /**
   * Internal function to compute the interior regular equivalence relative to the given one for a
   * network with categorical data on links. Space: O(n), Runtime: O(n + m log n) worst case
   *
   *
   * The result is returned in the state object.
   *
   * @param g
   *          a network
   * @param state
   ...
   * @param comparatorExtension
   */
  private static void coarsestRefiningEquivalenceRelativeToExtended(final Network g,
      final EquivalenceAlgorithmState state, final int[] equivalenceRelativeTo, Direction dir,
      Comparator<Relationship> comparatorExtension) {

    // number of vertices
    final int n = equivalenceRelativeTo.length;
    refiningRegularEquivalenceComparatorExtensionImpl(n,
        NetworkView.fromNetworkRelation(g, dir),
        Mappings.wrapUnmodifiableInt(equivalenceRelativeTo), comparatorExtension, state);

  }

  private static <T> void refiningRegularEquivalenceComparatorExtensionImpl(final int n,
  ...
  private static <V, T extends V, U extends V> void refiningRegularEquivalenceImpl(final int n,
    ...
    refiningRegularEquivalenceComparatorExtensionImpl(n,
        // only those methods are implemented that are actually need by the called algorithm
        new NetworkView<T, U>() {
  ...
  /**
   * Computes the maximum relative exact equivalence for a network with categorical data on links.
   * Space: O(n+m), Runtime: O(n+m) worst case.
   *
   * @param g
   *          a network
   * @param equivalenceRelativeTo
   ...
   *         p[i] <= max_{1<=j<=i-1} p[j] + 1
   */
  public static int[] maximumRelativeExactEquivalence(final Network g,
      final int[] equivalenceRelativeTo, Direction dir, int[] edgeClasses) {
    checkEquivalenceFormat(equivalenceRelativeTo, EQUIVALENCE_RELATIVE_TO);

    final int[] resEquivalence = new int[equivalenceRelativeTo.length];
    final int n = g.countMonadicIndices();
    final int[] counts = new int[2 * g.asRelation().countRelationships() + n];
    counts[0] = n;

    final EquivalenceAlgorithmState state = new EquivalenceAlgorithmState(resEquivalence, counts,
        2 * g.asRelation().countRelationships() + n, 1);

    coarsestRefiningExactEquivalenceRelativeTo(g, state, equivalenceRelativeTo, dir, edgeClasses);
   ...
   * with categorical data on links. Space: O(n+m), Runtime: O(n + m log n) worst case.
   *
   * @param g
   *          a network
   * @param equivalenceRelativeTo
   ...
   *         p[i] <= max_{1<=j<=i-1} p[j] + 1
   */
  public static int[] maximumRefiningRelativeExactEquivalence(final Network g,
      final int[] equivalenceRelativeTo, final int[] equivalenceToRefine, Direction dir,
      int[] edgeClasses) {
    checkEquivalenceFormat(equivalenceRelativeTo, EQUIVALENCE_RELATIVE_TO);
    checkEquivalenceFormat(equivalenceToRefine, EQUIVALENCE_TO_REFINE);
    ...
    final int[] resEquivalence = Arrays.copyOf(equivalenceToRefine, equivalenceToRefine.length);
    final int n = g.asRelation().countUnionDomain();
    final int[] counts = new int[2 * g.asRelation().countRelationships() + n];
    ...
    final EquivalenceAlgorithmState state = new EquivalenceAlgorithmState(resEquivalence, counts,
        2 * g.asRelation().countRelationships() + n, maxColor + 1);
  ...
  /**
   * Internal function to compute the exact role equivalence relative to the given one when
   * different edge classes are given. Space: O(n), Runtime: O(n + m log n) worst case
   *
   *
   * The result is returned in the state object.
   *
   ...
   *          the algorithm here assumes that neighborhoods are sorted increasingly by edge class
   */
  private static void coarsestRefiningExactEquivalenceRelativeTo(final Network g,
      final EquivalenceAlgorithmState state, final int[] equivalenceRelativeTo, Direction dir,
      int[] edgeClasses) {
    // number of vertices
    final int n = equivalenceRelativeTo.length;
    coarsestRefiningExactEquivalenceRelativeTo(n, state, equivalenceRelativeTo, edgeClasses,
        NetworkView.fromNetworkRelation(g, dir));
  }

  private static <T> void coarsestRefiningExactEquivalenceRelativeTo(final int n,
      final EquivalenceAlgorithmState state, final int[] equivalenceRelativeTo, int[] edgeClasses,
    ...
    int maxNumberVerticesForColor = 0;
    for (int begin = 0; begin < n;) {
      int expectedcolor = equivalenceRelativeTo[verticesOrderedByColor[begin]];
      int end = begin + 1;
      for (int iv = end; iv < n; end = ++iv) {
        final int v = verticesOrderedByColor[iv];
        if (equivalenceRelativeTo[v] != expectedcolor) {
          break;
        }
      }
      maxNumberVerticesForColor = Math.max(maxNumberVerticesForColor, end - begin);
    ...
    while (begin < n) {
      ...
      if (!heap.isEmpty()) {
        int iv = heap.peek();
        expectedclass = edgeClasses[positionView.inverseTieIndex(verticesOrderedByColor[iv],
            firstRelationships[iv - begin])];
      }
      // while we still have some unprocessed edges for vertices of this
      // color
      while (!heap.isEmpty()) {
        ...
        if (!heap.isEmpty()) {
          int iz = heap.peek();
          nextclass = edgeClasses[positionView.inverseTieIndex(verticesOrderedByColor[iz],
              firstRelationships[iz - begin])];
        }
        if (heap.isEmpty() || nextclass != expectedclass) {
          expectedclass = nextclass;
          for (int i = 0; i < state.getProcessedList().size(); ++i) {
   ...
   */
  private static void checkEquivalenceFormat(final int[] equivalence, final String argumentName) {
    int upperLimit = 0;
