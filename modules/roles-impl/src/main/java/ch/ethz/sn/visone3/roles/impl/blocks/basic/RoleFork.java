...
class RoleFork<T> implements RoleOperator<T> {

  public RoleFork(RoleOperator<T> first, RoleOperator<T> second, RoleReducer<T> combinator) {
    this.first = first;
    this.second = second;
  ...
  public T relative(T in) {
    ...
    T result = combinator.combine(getLeft(in), getRight(in));
    if (isConstant()) {
      cache = result;
      first.releaseCache();
      second.releaseCache();
