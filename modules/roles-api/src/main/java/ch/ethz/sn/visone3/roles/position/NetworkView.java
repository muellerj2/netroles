...
public interface NetworkView<T, U> extends TransposableNetworkView<T, U> {
   ...
   * 
   * @param node
   *          the node whose incident ties are requested.
   ...
   * 
   * @param node
   *          the node whose incident backward ties are requested.
   * @return the incident backward ties of {@code node}.
   */
  Iterable<? extends U> inverseTies(int node);

  /**
   * Determines the target of a (forward) tie incident to the specified source node.
   * 
   * @param node
  ...
  /**
   * Determines the target of a backward tie incident to the specified source node.
   * 
   * @param node
   *          the node whose incident ties are requested
   * @param tie
  ...
  /**
   * Determines a unique (but not necessarily consecutive) index of the (forward) tie {@code tie}
   * incident to {@code node}
   * 
   * @param node
   *          the source node of the incident tie.
   * @param tie
  ...
  /**
   * Determines a unique (but not necessarily consecutive) index of the backward tie {@code tie}
   * incident to {@code node}
   * 
   * @param node
   *          the source node of the incident tie.
  ...
  /**
   * Constructs a view on the network positions from a network's relational perspective.
   * 
   * @param network
   *          the network.
   * @param direction
   *          the direction used to view the position of nodes.
   * @return the view on the node positions for the given direction.
   */
  public static NetworkView<Relationship, Relationship> fromNetworkRelation(Network network,
      Direction direction) {
    final Relation rel = network.asRelation();
    boolean directed = network.isDirected();
    int uniqueTiesCount = directed ? rel.countRelationships()
        : 2 * network.asUndirectedGraph().countEdges();
    switch (direction) {
      case INCOMING:
